...
export default async function AdminQuestionBankPage() {
  ...
  return (
    <AdminShell context={context} title="Question Bank" description="Review prompts, answer keys, solutions, figure crops, and extraction quality.">
      ...
      <div className="overflow-x-auto rounded-md border border-white/10 bg-white/5">
        <table className="w-full min-w-[900px] text-left text-sm">
          <thead className="bg-[#061117]/70 text-xs uppercase text-white/50">
            <tr>{["Source", "Subject", "Number", "Type", "Topic", "Solution"].map((header) => <th key={header} className="border-b border-white/10 px-4 py-3">{header}</th>)}</tr>
          </thead>
          <tbody>
            {questions.slice(0, 100).map((question) => (
              <tr key={question.id} className="border-b border-white/10">
                <td className="px-4 py-3 text-white/75">{question.source}</td>
